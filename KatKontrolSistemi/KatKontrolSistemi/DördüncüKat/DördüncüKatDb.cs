...
namespace KatKontrolSistemi.DördüncüKat
{
    class DördüncüKatDb
    {
        ...
        {
            string sql = "SELECT * FROM PersonelTanitimKarti WHERE KatId='4' AND PanelId='7' AND OdaId='1'";
            DataTable Tablo = Dbase.TabloCek(sql);
            return Tablo;
        }
        public DataTable Panel8Oda1KisiListesi()
        {
            string sql = "SELECT * FROM PersonelTanitimKarti WHERE KatId='4' AND PanelId='8' AND OdaId='1'";
            DataTable Tablo = Dbase.TabloCek(sql);
            return Tablo;
        }
        public DataTable Panel8Oda2KisiListesi()
        {
            string sql = "SELECT * FROM PersonelTanitimKarti WHERE KatId='4' AND PanelId='8' AND OdaId='2'";
            DataTable Tablo = Dbase.TabloCek(sql);
            return Tablo;
        }
        public DataTable Panel8Oda3KisiListesi()
        {
            string sql = "SELECT * FROM PersonelTanitimKarti WHERE KatId='4' AND PanelId='8' AND OdaId='3'";
            DataTable Tablo = Dbase.TabloCek(sql);
            return Tablo;
        }
    }
